...
C. EDICIÓN DE DATOS (ARQUITECTURA DE "BORRADOR")
    ...
    - Pila de Deshacer (Undo): El sistema implementa una pila de deshacer (`session['history']`) con un límite de 15 cambios.
        ...
        - (Documentación de Google) Restauración de Posición (v7.7): Al deshacer un 'borrado', la API re-inserta la fila en su posición (índice) original en la cuadrícula, no al final de la lista.
        - (Documentación de Google) Scroll Inteligente (v7.28): Al deshacer cualquier acción, la vista de la tabla se desplaza automáticamente a la fila afectada (`affected_row_id`), incluso si está fuera de la vista.
...
La aplicación utiliza una arquitectura de sesión en el servidor (Flask-Session) para gestionar los datos del usuario de forma no destructiva.
  ...
  C. GESTIÓN DE ESTADO (EN SESIÓN):
    ...
    2. session['history'] (La Pila de Deshacer)
        ...
        - (Documentación de Google) Ejemplos de objetos:
